          ...
          <div className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-gray-600 mt-1">Manage your A/C Clinic services and account</p>
              </div>
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{user.pendingServices}</div>
                    <p className="text-xs text-muted-foreground">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Math.floor((new Date().getTime() - new Date(user.joinDate).getTime()) / (1000 * 60 * 60 * 24 * 30))}mo
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Valued customer
                    </p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <Button variant="outline" className="w-full h-20 flex flex-col" onClick={() => setActiveTab("payments")}>
                        <CreditCard className="h-6 w-6 mb-2" />
                        <span>Make Payment</span>
                      </Button>
                      <Button variant="outline" className="w-full h-20 flex flex-col" onClick={() => setActiveTab("bookings")}>
                      ...
                      {recentBookings.filter(b => b.status === "confirmed").map((booking) => (
                        <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Technician: {booking.technician}</p>
                          </div>
                      ...
                      <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-4">
                          <div className="p-2 bg-primary/10 rounded-lg">
                            {booking.service.includes("HVAC") && <Snowflake className="h-5 w-5 text-primary" />}
                            {booking.service.includes("Refrigeration") && <Building className="h-5 w-5 text-primary" />}
                            {booking.service.includes("Medical") && <Heart className="h-5 w-5 text-primary" />}
                            {!booking.service.includes("HVAC") && !booking.service.includes("Refrigeration") && !booking.service.includes("Medical") && <Wrench className="h-5 w-5 text-primary" />}
                          </div>
                          <div>
                            <p className="font-medium">{booking.service}</p>
                            <p className="text-sm text-gray-600">{booking.date} • ${booking.amount}</p>
                          </div>
                        </div>
                        {getStatusBadge(booking.status)}
                      </div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-green-600">${payments.filter(p => p.status === "paid").reduce((sum, p) => sum + p.amount, 0)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Pending Payments</span>
                        <span className="font-bold text-yellow-600">${payments.filter(p => p.status === "pending").reduce((sum, p) => sum + p.amount, 0)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                    ...
                    {payments.filter(p => p.status === "pending").length > 0 ? (
                      <div className="space-y-4">
                        {payments.filter(p => p.status === "pending").map((payment) => (
                          <div key={payment.id} className="flex justify-between items-center p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            <div>
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {payment.date} • {payment.method} • {payment.bookingId}
                            </p>
                          </div>
                        </div>
